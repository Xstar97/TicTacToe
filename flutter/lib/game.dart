...
class _GamePageLocalState extends State<GamePageLocal> {
  ...
  void dialogWinningMessage(BuildContext context, String playerChar){
    showDialog<void>(
      ...
      builder: (BuildContext alert) {
        return AlertDialog(
          title: Center(child: Text(S.of(context).gameMessageOverTitleWinner),),
          content: Text(S.of(context).gameMessageOverMessageWinner(playerChar),),
          actions: <Widget>[
            FlatButton(
  ...
  void dialogMoveInUsedMessage(BuildContext context){
    showDialog<void>(
      ...
      builder: (BuildContext alert) {
        return AlertDialog(
          title: Center(child: Text(S.of(context).gameMessageReallyTitle),),
          content: Text(S.of(context).gameMessageReallyMessage),
          actions: <Widget>[
            FlatButton(
              child: Text(S.of(context).gameActionContinue),
              onPressed: () {
                Navigator.of(alert).pop();
              },
            ),
  ...
  void dialogGameOverMessageNoMoreMoves(){
    showDialog<void>(
      ...
      builder: (BuildContext alert) {
        return AlertDialog(
          title: Center(child: Text(S.of(context).gameMessageOverTitle),),
          content: Text(S.of(context).gameMessageOverMessageTie,),
          actions: <Widget>[
            FlatButton(
              child: Text(S.of(context).gameActionNew),
              onPressed: () {
